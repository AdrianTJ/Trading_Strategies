...
def run_simulations(initial_cash: float = 10000.0):
    ...
    for asset in ASSETS:
        ...
        for strat_name, signal_func in STRATEGIES.items():
            ...
            save_simulation(session, sim_df, strat_name, asset, initial_cash, rf_rate, is_benchmark=False)
            print(f"Finished {strat_name} on {asset}.")

    # 2. Benchmarks
    if 'sp500' in df.columns and 'treasury_10y' in df.columns:
        print("Generating benchmarks...")
        
        # Calculate bond returns (duration approximation)
        y_today = df['treasury_10y'] / 100.0
        y_yesterday = y_today.shift(1)
        bond_returns = (y_yesterday / 252) - 8.8 * (y_today - y_yesterday)
        bond_returns.iloc[0] = 0
        
        # S&P 500 returns
        sp500_returns = df['sp500'].pct_change().fillna(0)
        
        # 60/40 returns
        portfolio_60_40_returns = 0.6 * sp500_returns + 0.4 * bond_returns
        
        # Generate price indexes starting at 100.0
        sp500_index = (1 + sp500_returns).cumprod() * 100.0
        portfolio_60_40_index = (1 + portfolio_60_40_returns).cumprod() * 100.0
        
        # S&P 500 Buy & Hold Benchmark
        asset_df_sp500 = pd.DataFrame({'close': sp500_index, 'cpi': df['cpi']}, index=df.index)
        signals = generate_lump_sum_signals(asset_df_sp500)
        sim_df_sp500 = Simulator(initial_cash=initial_cash).run(asset_df_sp500, signals)
        save_simulation(session, sim_df_sp500, "Buy & Hold", "S&P 500", initial_cash, rf_rate, is_benchmark=True)
        
        # 60/40 Portfolio Benchmark
        asset_df_6040 = pd.DataFrame({'close': portfolio_60_40_index, 'cpi': df['cpi']}, index=df.index)
        signals = generate_lump_sum_signals(asset_df_6040)
        sim_df_6040 = Simulator(initial_cash=initial_cash).run(asset_df_6040, signals)
        save_simulation(session, sim_df_6040, "60/40 Portfolio", "Balanced", initial_cash, rf_rate, is_benchmark=True)
